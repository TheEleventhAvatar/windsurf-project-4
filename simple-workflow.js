...
class SimpleFailureSimulator {
  constructor(token) {
    this.agent = new SimpleGitHubAgent(token);
    this.scenarios = [];
  }
  ...
  async runCaseA_PermissionChange(repoName, owner = 'octocat') {
  ...
  async runCaseB_StaleState(repoName, owner = 'octocat') {
  ...
  async runCaseC_APIError(repoName, owner = 'octocat') {
...
module.exports = SimpleFailureSimulator;
